...
export default class AsideLeft {
    ...
    emitEventToCreateRelevantFormByDrawningBoard(event) {
        if(event.target === this.asideLeftElement) return;
        let goodTargetEvent = event.target;
        !goodTargetEvent.className.baseVal && (goodTargetEvent = goodTargetEvent.parentElement);
        const className = goodTargetEvent.className.baseVal;
        const shapeName = className.split(/(?=[A-Z])/)[0];
        this.clearAllToolsColors();
        this.changeColorToTheSelectedTool(goodTargetEvent);
        bus.emit('createFormForSelectedShape' , shapeName)
    }
    ...
    changeColorToTheSelectedTool(goodTargetEvent) {
        if(goodTargetEvent.className.baseVal !== "pencilTool") {
            goodTargetEvent.firstElementChild.style.stroke = "white";
            goodTargetEvent.lastElementChild.style.stroke = "white";
        } else {
            goodTargetEvent.firstElementChild.style.fill = "white";
        }
        goodTargetEvent.parentElement.style.backgroundColor = "#381620";
        goodTargetEvent.parentElement.style.borderColor = "#381620";
    }
}
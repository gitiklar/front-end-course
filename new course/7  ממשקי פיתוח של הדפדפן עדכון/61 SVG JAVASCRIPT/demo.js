...
for(let i=0;i<10;i++) {
    ...
    circle.setAttributeNS(null,'cx',30*i);
    circle.setAttributeNS(null,'cy',30*i);
    circle.setAttributeNS(null,'r',15);
    circle.classList.add('click-able');
    circle.style.fill = colors[i%colors.length];
    svg.appendChild(circle);
}

svg.addEventListener('click',()=>{
    const shape = event.target;
    if(!shape.classList.contains('click-able')) return;
    shape.setAttributeNS(null,'cy',310);
});
